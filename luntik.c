...
void my_printf(const char *format, ...) {
    ...
    char *buffer = (char*)malloc(1024); // Буфер для результата
    if (buffer == NULL) return; // Обработка ошибки выделения памяти
    int buffer_index = 0;
    for (int i = 0; format[i] != '\0'; i++) {
        if (format[i] == '%') {
            ...
            switch (format[i]) {
                ...
                case 'c': {
                    char c = va_arg(args, int); // int используется, поскольку char может быть повышен до int
                ...
                case 'x': {
                    ...
                    char x_str[32];
                    i_to_base(x, 16, x_str); // Преобразование в основание 160
                    strcpy(buffer + buffer_index, x_str);
                    buffer_index += strlen(x_str);
                    break;
                }
    ...
    write(1, buffer, buffer_index); // Вывод в стандартный вывод
